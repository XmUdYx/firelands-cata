...
void WorldSession::HandleCharEnum(PreparedQueryResult result)
{
    ...
    if (result)
    {
        do
        {
            ...
            {
                LOG_ERROR("entities.player.loading",
                    "Player %s has wrong Appearance values (Hair/Skin/Color), "
                    "forcing recustomize",
                    charInfo.Guid.ToString().c_str());

                // Make sure customization always works properly - send all zeroes
                // instead
                charInfo.SkinID = 0;
                charInfo.FaceID = 0;
            ...
            if (!sCharacterCache->HasCharacterCacheEntry(charInfo.Guid)) // This can happen if characters are inserted
                                                                         // into the database manually. Core hasn't loaded
                                                                         // name data yet.
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    recvData >> createInfo->Name >> createInfo->Race >> createInfo->Class >> createInfo->Gender >> createInfo->Skin >> createInfo->Face >> createInfo->HairStyle >> createInfo->HairColor >>
    ...
    {
        LOG_ERROR("network",
            "Class (%u) not found in DBC while creating new char for "
            "account (ID: %u): wrong DBC files or cheater?",
            createInfo->Class, GetAccountId());
    ...
    {
        LOG_ERROR("network",
            "Race (%u) not found in DBC while creating new char for "
            "account (ID: %u): wrong DBC files or cheater?",
            createInfo->Race, GetAccountId());
        SendCharCreate(CHAR_CREATE_FAILED);
        return;
    }
    ...
    {
        LOG_ERROR("entities.player.cheat",
            "Expansion %u account:[%d] tried to Create character with "
            "expansion %u race (%u)",
            GetAccountExpansion(), GetAccountId(), raceEntry->Race_related, createInfo->Race);
    ...
    if (classEntry->Required_expansion > GetAccountExpansion())
    {
        LOG_ERROR("entities.player.cheat",
            "Expansion %u account:[%d] tried to Create character with "
            "expansion %u class (%u)",
            GetAccountExpansion(), GetAccountId(), classEntry->Required_expansion, createInfo->Class);
    ...
    {
        LOG_INFO("entities.player.cheat", "Account:[%d] but tried to Create character with empty [name] ", GetAccountId());
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt)
                                    .WithChainingPreparedCallback(
                                        [this](QueryCallback& queryCallback, PreparedQueryResult result)
                                        {
                                            if (result)
                                            {
                                    ...
                                    .WithChainingPreparedCallback(
                                        [this, createInfo](QueryCallback& queryCallback, PreparedQueryResult result)
                                        {
                                            if (result)
                                            {
                                                Field* fields = result->Fetch();
                                                createInfo->CharCount = uint8(fields[0].GetUInt64()); // SQL's COUNT() returns uint64 but it
                                                                                                      // will always be less than uint8.Max
                                            ...
                                            std::function<void(PreparedQueryResult)> finalizeCharacterCreation = [this, createInfo](PreparedQueryResult result)
                                            {
                                                ...
                                                if (result)
                                                {
                                                    ...
                                                    /// @todo what to if account already has characters of both
                                                    /// races?
                                                    ...
                                                    /// @todo check if cinematic already shown? (already logged
                                                    /// in?; cinematic field)
                                                ...
                                                // Check name uniqueness in the same step as saving to
                                                // database
                                                ...
                                                std::shared_ptr<Player> newChar(new Player(this),
                                                    [](Player* ptr)
                                                    {
                                                ...
                                                AddTransactionCallback(CharacterDatabase.AsyncCommitTransaction(characterTransaction))
                                                    .AfterComplete(
                                                        [this, newChar = std::move(newChar)](bool success)
                                                        {
                                                            if (success)
                                                            {
                                                                LOG_INFO("entities.player.character",
                                                                    "Account: %d (IP: %s) Create "
                                                                    "Character:[%s] (GUID: %u)",
                                                                    GetAccountId(), GetRemoteAddress().c_str(), newChar->GetName().c_str(), newChar->GetGUID().GetCounter());
                                                                sScriptMgr->OnPlayerCreate(newChar.get());
                                                                sCharacterCache->AddCharacterCacheEntry(newChar->GetGUID(), GetAccountId(), newChar->GetName(),
                                                                    newChar->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER), newChar->getRace(), newChar->getClass(), newChar->getLevel());
...
void WorldSession::HandleCharDeleteOpcode(WorldPackets::Character::CharDelete& packet)
{
    ...
    LOG_INFO("entities.player.character", "Account: %d, IP: %s deleted character: %s, %s, Level: %u", accountId, GetRemoteAddress().c_str(), name.c_str(), packet.Guid.ToString().c_str(), level);

    // To prevent hook failure, place hook before removing reference from DB
    sScriptMgr->OnPlayerDelete(packet.Guid, initAccountId); // To prevent race conditioning, but as it
                                                            // also makes sense, we hand the accountId
                                                            // over for successful delete.
    // Shouldn't interfere with character deletion though

    if (sLog->ShouldLog("entities.player.dump",
            LOG_LEVEL_INFO)) // optimize GetPlayerDump call
    {
        std::string dump;
...
void WorldSession::HandlePlayerLoginOpcode(WorldPackets::Character::PlayerLogin& packet)
{
    ...
    {
        LOG_ERROR("network", "Account (%u) can't login with that character (%s).", GetAccountId(), packet.Guid.ToString().c_str());
        KickPlayer();
        return;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    // guild_member WHERE guid = '%u'", pCurrChar->GetGUID().GetCounter());
    if (PreparedQueryResult resultGuild = holder.GetPreparedResult(PLAYER_LOGIN_QUERY_LOAD_GUILD))
    {
        ...
        if (Guild* guild = sGuildMgr->GetGuildById(pCurrChar->GetGuildId()))
        {
            ...
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_LEVEL, 0, 0, 0, nullptr, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL, 0, 0, 0, nullptr, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION, 0, 0, 0, nullptr, pCurrChar);
            guild->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GAIN_EXALTED_REPUTATION, 0, 0, 0, nullptr, pCurrChar);
        }
    }
    else if (pCurrChar->GetGuildId()) // clear guild related fields in case
                                      // wrong data about non existed membership
    {
    ...
    if (pCurrChar->GetGuildId() != 0)
    {
        ...
        else
        {
            ...
            LOG_ERROR("misc",
                "Player %s (GUID: %u) marked as member of not existing "
                "guild (id: %u), removing guild membership for player.",
                pCurrChar->GetName().c_str(), pCurrChar->GetGUID().GetCounter(), pCurrChar->GetGuildId());
            pCurrChar->SetInGuild(0);
        }
    }
    ...
    // Load pet if any (if player not alive and in taxi flight or another then pet
    // will remember as temporary unsummoned)
    pCurrChar->LoadPet();
    ...
    if (pCurrChar->HasAtLoginFlag(AT_LOGIN_RESET_TALENTS))
    {
        ...
        pCurrChar->SendTalentsInfoData(false); // original talents send already in to
                                               // SendInitialPacketsBeforeAddToMap, resend reset state
    ...
    if (firstLogin)
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_START_ALL_REP))
        {
            ...
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(942), 42999,
                false); // Cenarion Expedition
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(935), 42999,
                false); // The Sha'tar
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(936), 42999,
                false); // Shattrath City
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1011), 42999,
                false); // Lower City
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(970), 42999,
                false); // Sporeggar
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(967), 42999,
                false); // The Violet Eye
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(989), 42999,
                false); // Keepers of Time
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(932), 42999,
                false); // The Aldor
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(934), 42999,
                false); // The Scryers
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1038), 42999,
                false); // Ogri'la
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1077), 42999,
                false); // Shattered Sun Offensive
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1106), 42999,
                false); // Argent Crusade
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1104), 42999,
                false); // Frenzyheart Tribe
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1090), 42999,
                false); // Kirin Tor
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1098), 42999,
                false); // Knights of the Ebon Blade
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1156), 42999,
                false); // The Ashen Verdict
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1073), 42999,
                false); // The Kalu'ak
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1105), 42999,
                false); // The Oracles
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1119), 42999,
                false); // The Sons of Hodir
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1091), 42999,
                false); // The Wyrmrest Accord
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1204), 42999,
                false); // Avengers of Hyjal
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1158), 42999,
                false); // Guardians of Hyjal
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1173), 42999,
                false); // Ramkahen
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1171), 42999,
                false); // Therazane
            repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1135), 42999,
                false); // The Earthen Ring

            // Factions depending on team, like cities and some more stuff
            switch (pCurrChar->GetTeam())
            {
            case ALLIANCE:
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(72), 42999,
                    false); // Stormwind
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(47), 42999,
                    false); // Ironforge
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(69), 42999,
                    false); // Darnassus
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(930), 42999,
                    false); // Exodar
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(730), 42999,
                    false); // Stormpike Guard
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(978), 42999,
                    false); // Kurenai
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(54), 42999,
                    false); // Gnomeregan Exiles
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(946), 42999,
                    false); // Honor Hold
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1037), 42999,
                    false); // Alliance Vanguard
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1068), 42999,
                    false); // Explorers' League
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1126), 42999,
                    false); // The Frostborn
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1094), 42999,
                    false); // The Silver Covenant
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1050), 42999,
                    false); // Valiance Expedition
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1177), 42999,
                    false); // Baradin's Wardens
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1174), 42999,
                    false); // Wildhammer Clan
                break;
            case HORDE:
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(76), 42999,
                    false); // Orgrimmar
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(68), 42999,
                    false); // Undercity
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(81), 42999,
                    false); // Thunder Bluff
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(911), 42999,
                    false); // Silvermoon City
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(729), 42999,
                    false); // Frostwolf Clan
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(941), 42999,
                    false); // The Mag'har
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(530), 42999,
                    false); // Darkspear Trolls
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(947), 42999,
                    false); // Thrallmar
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1052), 42999,
                    false); // Horde Expedition
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1067), 42999,
                    false); // The Hand of Vengeance
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1124), 42999,
                    false); // The Sunreavers
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1064), 42999,
                    false); // The Taunka
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1085), 42999,
                    false); // Warsong Offensive
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1172), 42999,
                    false); // Dragonmaw Clan
                repMgr.SetOneFactionReputation(sFactionStore.LookupEntry(1178), 42999,
                    false); // Hellscream's Reach
                break;
            default:
                break;
            }
    ...
    LOG_INFO("entities.player.character", "Account: %d (IP: %s) Login Character:[%s] (GUID: %u) Level: %d", GetAccountId(), IP_str.c_str(), pCurrChar->GetName().c_str(),
        pCurrChar->GetGUID().GetCounter(), pCurrChar->getLevel());
    ...
    // if we're loading a dead player, repop them to the GY after the load is
    // finished
    ...
    FC_METRIC_EVENT("player_events", "Login", pCurrChar->GetName());
}

void WorldSession::HandleSetFactionAtWar(WorldPacket& recvData)
{
    LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_ATWAR");
...
// I think this function is never used :/ I dunno, but i guess this opcode not
// exists
void WorldSession::HandleSetFactionCheat(WorldPacket& /*recvData*/)
{
    LOG_ERROR("network", "WORLD SESSION: HandleSetFactionCheat, not expected "
                         "call, please report.");
...
{
    LOG_DEBUG("network", "WORLD: Received CMSG_SET_WATCHED_FACTION");
    uint32 fact;
    recvData >> fact;
...
{
    LOG_DEBUG("network", "WORLD: Received CMSG_SET_FACTION_INACTIVE");
    uint32 replistid;
    uint8 inactive;
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    ...
    // Ensure that the character belongs to the current account, that rename at
    // login is enabled and that there is no character with the desired new name
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(&WorldSession::HandleCharRenameCallback, this, renameInfo, std::placeholders::_1)));
}
...
void WorldSession::HandleCharRenameCallback(std::shared_ptr<CharacterRenameInfo> renameInfo, PreparedQueryResult result)
{
    ...
    LOG_INFO("entities.player.character", "Account: %d (IP: %s) Character:[%s] (%s) Changed name to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(),
        renameInfo->Guid.ToString().c_str(), renameInfo->Name.c_str());
...
void WorldSession::HandleAlterAppearance(WorldPacket& recvData)
{
    ...
    if (!Player::ValidateAppearance(_player->getRace(), _player->getClass(), _player->GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_GENDER), bs_hair->Data, Color,
            _player->GetByteValue(PLAYER_BYTES, PLAYER_BYTES_OFFSET_FACE_ID), bs_facialHair->Data,
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    if (!IsLegitCharacterForAccount(customizeInfo->Guid))
    {
        LOG_INFO("entities.player.cheat",
            "Account %u, IP: %s tried to customise %s, but it does not "
            "belong to their account!",
    ...
    recvData >> customizeInfo->Name >> customizeInfo->Gender >> customizeInfo->Skin >> customizeInfo->HairColor >> customizeInfo->HairStyle >> customizeInfo->FacialHair >> customizeInfo->Face;
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(&WorldSession::HandleCharCustomizeCallback, this, customizeInfo, std::placeholders::_1)));
}
...
void WorldSession::HandleCharCustomizeCallback(std::shared_ptr<CharacterCustomizeInfo> customizeInfo, PreparedQueryResult result)
{
    ...
    LOG_INFO("entities.player.character", "Account: %d (IP: %s), Character[%s] (%s) Customized to: %s", GetAccountId(), GetRemoteAddress().c_str(), oldName.c_str(),
        customizeInfo->Guid.ToString().c_str(), customizeInfo->Name.c_str());
}
...
void WorldSession::HandleEquipmentSetSave(WorldPacket& recvData)
{
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        // equipment manager sends "1" (as raw GUID) for slots set to "ignore"
        // (don't touch slot at equip set)
        if (itemGuid.GetRawValue() == 1)
        {
            // ignored slots saved as bit mask because we have no free special values
            // for Items[i]
...
void WorldSession::HandleEquipmentSetUse(WorldPacket& recvData)
{
    ...
    for (uint32 i = 0; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        LOG_DEBUG("entities.player.items", "%s: srcbag %u, srcslot %u", itemGuid.ToString().c_str(), srcbag, srcslot);

        // check if item slot is set to "ignored" (raw value == 1), must not be
        // unequipped then
        if (itemGuid.GetRawValue() == 1)
            continue;
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    {
        LOG_INFO("entities.player.cheat",
            "Account %u, IP: %s tried to factionchange character %s, but "
            "it does not belong to their account!",
    ...
    recvData >> factionChangeInfo->Name >> factionChangeInfo->Gender >> factionChangeInfo->Skin >> factionChangeInfo->HairColor >> factionChangeInfo->HairStyle >> factionChangeInfo->FacialHair >>
        factionChangeInfo->Face >> factionChangeInfo->Race;
    ...
    _queryProcessor.AddCallback(
        CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(std::bind(&WorldSession::HandleCharFactionOrRaceChangeCallback, this, factionChangeInfo, std::placeholders::_1)));
}
...
void WorldSession::HandleCharFactionOrRaceChangeCallback(std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo, PreparedQueryResult result)
{
    ...
    uint32 playerGroupId = !fields[2].IsNull() ? fields[2].GetUInt32() : 0;
    ...
    if (oldRace != factionChangeInfo->Race)
    {
        ...
        if (factionChangeInfo->FactionChange)
        {
            ...
            if (playerGroupId && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP))
            {
                if (Group* group = sGroupMgr->GetGroupByDbStoreId(playerGroupId))
                {
            ...
            // Mark all rewarded quests as "active" (will count for completed quests
            // achievements)
