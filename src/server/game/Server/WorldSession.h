...
enum ChangeDynamicDifficultyResult {
  ERR_DIFFICULTY_CHANGE_COOLDOWN_S =
      0,  // sends the remaining time in seconds to the client
  ...
  ERR_DIFFICULTY_CHANGE_OTHER_HEROIC_S =
      8,  // sends it when someone is locked for the encounter on heroic, sends
          // the locked player packed guid
  ...
  ERR_DIFFICULTY_CHANGE_UPDATE_MAP_DIFFICULTY_ENTRY =
      7,  // sends the ID of MapDifficulty
  ERR_DIFFICULTY_CHANGE_SUCCESS =
      11  // sends remaining time in time_t and mapId
};
...
class WorldSessionFilter : public PacketFilter {
 public:
  explicit WorldSessionFilter(WorldSession* pSession)
      : PacketFilter(pSession) {}
  ~WorldSessionFilter() {}
...
class FC_GAME_API WorldSession {
 public:
  ...
  void SendPartyResult(PartyOperation operation, std::string const& member,
  ...
  void InitializeSessionCallback(
  ...
  // Spell
  void HandleClientCastFlags(WorldPacket& recvPacket, uint8 castFlags,
  ...
  void SendPetSlotUpdated(int32 petNumberA, int32 petSlotA, int32 petNumberB,
                          int32 petSlotB);
  void SendPetAdded(int32 petSlot, int32 petNumber, int32 creatureID,
                    int32 level, std::string name);
  ...
  void SendAuctionCommandResult(AuctionEntry* auction, uint32 Action,
                                uint32 ErrorCode, uint32 bidError = 0);
  void SendAuctionBidderNotification(uint32 location, uint32 auctionId,
                                     ObjectGuid bidder, uint64 bidSum,
                                     uint64 diff, uint32 item_template);
  void SendAuctionOwnerNotification(AuctionEntry* auction);
  void SendAuctionRemovedNotification(uint32 auctionId, uint32 itemEntry,
                                      int32 randomPropertyId);

  // Item Enchantment
  void SendEnchantmentLog(ObjectGuid target, ObjectGuid caster, uint32 itemId,
                          uint32 enchantId);
  void SendItemEnchantTimeUpdate(ObjectGuid Playerguid, ObjectGuid Itemguid,
  ...
  // Guild/Arena Team
  void SendArenaTeamCommandResult(uint32 team_action, std::string const& team,
  ...
  char const* GetFirelandsString(uint32 entry) const;
  ...
  void UpdateTimeOutTime(uint32 diff) { m_timeOutTime -= int32(diff); }
 ...
 public:                                      // opcodes handlers
  ...
  void Handle_EarlyProccess(
      WorldPacket&
          recvPacket);  // just mark packets processed in WorldSocket::OnRead
  void Handle_ServerSide(WorldPacket& recvPacket);  // sever side only, can't be
                                                    // accepted from client
  void Handle_Deprecated(
      WorldPacket& recvPacket);  // never used anymore by client
  void LogUnprocessedTail(WorldPacket const* packet);

  void HandleCharEnum(PreparedQueryResult result);
  void HandleCharEnumOpcode(
      WorldPackets::Character::EnumCharacters& /*enumCharacters*/);
  ...
  void HandleLoadScreenOpcode(
      WorldPackets::Character::LoadingScreenNotify& packet);
  void HandlePlayerLogin(LoginQueryHolder const& holder);
  void HandleCharFactionOrRaceChange(WorldPacket& recvData);
  void HandleCharFactionOrRaceChangeCallback(
      std::shared_ptr<CharacterFactionChangeInfo> factionChangeInfo,
      PreparedQueryResult result);
  void HandleCharRenameOpcode(WorldPacket& recvData);
  void HandleCharRenameCallback(std::shared_ptr<CharacterRenameInfo> renameInfo,
                                PreparedQueryResult result);
  void HandleSetPlayerDeclinedNames(WorldPacket& recvData);
  void HandleAlterAppearance(WorldPacket& recvData);
  void HandleCharCustomize(WorldPacket& recvData);
  void HandleCharCustomizeCallback(
      std::shared_ptr<CharacterCustomizeInfo> customizeInfo,
      PreparedQueryResult result);
  void HandleOpeningCinematic(WorldPackets::Misc::OpeningCinematic& packet);

  void HandleRandomizeCharNameOpcode(
      WorldPackets::Character::GenerateRandomCharacterName& packet);
  void HandleReorderCharacters(WorldPacket& recvData);
  void SendCharCreate(ResponseCodes result);
  void SendCharDelete(ResponseCodes result);
  void SendCharRename(ResponseCodes result,
                      CharacterRenameInfo const* renameInfo);
  void SendCharCustomize(ResponseCodes result,
                         CharacterCustomizeInfo const* customizeInfo);
  void SendCharFactionChange(
      ResponseCodes result,
      CharacterFactionChangeInfo const* factionChangeInfo);
  void SendSetPlayerDeclinedNamesResult(DeclinedNameResult result,
                                        ObjectGuid guid);
  ...
  // new party stats
  void HandleRequestHonorStatsOpcode(
      WorldPackets::Inspect::RequestHonorStats& packet);
  void HandleRequestInspectRatedBgStatsOpcode(
      WorldPackets::Inspect::RequestInspectRatedBgStats& packet);
  ...
  void HandleSuspendTokenResponse(
      WorldPackets::Movement::SuspendTokenResponse& suspendTokenResponse);
  ...
  void HandlePartyInviteResponseOpcode(
      WorldPackets::Party::PartyInviteResponse& packet);
  ...
  void HandleSetEveryoneIsAssistant(
      WorldPackets::Party::SetEveryoneIsAssistant& packet);
  ...
  void HandleGuildGetRosterOpcode(
      WorldPackets::Guild::GuildGetRoster& /*packet*/);
  void HandleRequestGuildRewardsList(
      WorldPackets::Guild::RequestGuildRewardsList& /*packet*/);
  ...
  void HandleGuildReplaceGuildMaster(
      WorldPackets::Guild::ReplaceGuildMaster& packet);
  ...
  void HandleGuildQueryRecipesOpcode(
      WorldPackets::Guild::GuildQueryRecipes& packet);
  void HandleGuildQueryMembersForRecipe(
      WorldPackets::Guild::GuildQueryMembersForRecipe& packet);
  void HandleGuildQueryMemberRecipe(
      WorldPackets::Guild::GuildQueryMemberRecipes& packet);
  ...
  void HandleGuildRequestChallengeUpdate(
      WorldPackets::Guild::GuildChallengeUpdateRequest& packet);
  ...
  void HandleOpenWrappedItemCallback(uint16 pos, ObjectGuid itemGuid,
                                     PreparedQueryResult result);
  void HandleCastSpellOpcode(WorldPackets::Spells::CastSpell& castRequest);
  void HandleCancelCastOpcode(WorldPacket& recvPacket);
  void HandleCancelQueuedSpellOpcode(
      WorldPackets::Spells::CancelQueuedSpell& cancelQueuedSpell);
  ...
  void HandleQuestgiverStatusMultipleQuery(
      WorldPackets::Quest::QuestGiverStatusMultipleQuery& packet);
  void HandleQuestgiverHelloOpcode(
      WorldPackets::Quest::QuestGiverHello& packet);
  ...
  void HandleQuestgiverQueryQuestOpcode(
      WorldPackets::Quest::QuestGiverQueryQuest& packet);
  ...
  void HandleNextCinematicCamera(
      WorldPackets::Misc::NextCinematicCamera& packet);
  ...
  void HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellid,
                             uint16 flag, ObjectGuid guid2, float x, float y,
                             float z);
  ...
  void SendBfQueueInviteResponse(ObjectGuid guid, uint32 zoneId,
                                 bool canQueue = true, bool full = false);
  void SendBfEntered(ObjectGuid guid);
  void SendBfLeaveMessage(ObjectGuid guid,
  ...
  void HandleLfgProposalResultOpcode(
      WorldPackets::LFG::LFGProposalResponse& lfgProposalResponse);
  void HandleLfgSetRolesOpcode(WorldPackets::LFG::LFGSetRoles& lfgSetRoles);
  void HandleLfgSetCommentOpcode(
      WorldPackets::LFG::LFGSetComment& lfgSetComment);
  void HandleLfgSetBootVoteOpcode(
      WorldPackets::LFG::LFGBootPlayerVote& lfgBootPlayerVote);
  void HandleLfgTeleportOpcode(WorldPackets::LFG::LFGTeleport& lfgTeleport);
  void HandleLfgGetLockInfoOpcode(
      WorldPackets::LFG::LFGGetSystemInfo& lfgGetSystemInfo);
  ...
  // Guild Bank
  void HandleGuildPermissionsQuery(
      WorldPackets::Guild::GuildPermissionsQuery& packet);
  void HandleGuildBankMoneyWithdrawn(
      WorldPackets::Guild::GuildBankRemainingWithdrawMoneyQuery& packet);
  ...
  void HandleGuildBankDepositMoney(
      WorldPackets::Guild::GuildBankDepositMoney& packet);
  void HandleGuildBankWithdrawMoney(
      WorldPackets::Guild::GuildBankWithdrawMoney& packet);
  void HandleGuildBankSwapItems(
      WorldPackets::Guild::GuildBankSwapItems& packet);
  void HandleGuildBankUpdateTab(
      WorldPackets::Guild::GuildBankUpdateTab& packet);
  void HandleGuildBankBuyTab(WorldPackets::Guild::GuildBankBuyTab& packet);
  void HandleGuildBankTextQuery(
      WorldPackets::Guild::GuildBankTextQuery& packet);
  void HandleGuildBankSetTabText(
      WorldPackets::Guild::GuildBankSetTabText& packet);
  ...
  void HandleUpdateMissileTrajectory(
      WorldPackets::Spells::UpdateMissileTrajectory& updateMissileTrajectory);
  void HandleViolenceLevel(WorldPacket& recvPacket);
  void HandleObjectUpdateFailedOpcode(WorldPacket& recvPacket);
  void HandleRequestCategoryCooldowns(WorldPacket& recvPacket);
  void SendStreamingMovie();
  void HandleRequestResearchHistory(
      WorldPackets::Archaeology::RequestResearchHistory& packet);
 ...
 protected:
  class DosProtection {
   ...
   private:
    ...
    // mark this member as "mutable" so it can be modified even in const
    // functions
 ...
 private:
  ...
  // logging helper
  void LogUnexpectedOpcode(WorldPacket* packet, char const* status,
                           const char* reason);
  ...
  ObjectGuid::LowType m_GUIDLow;  // set logined or recently logout player
                                  // (while m_playerRecentlyLogout set)
  Player* _player;
  std::shared_ptr<WorldSocket> m_Socket;
  std::string m_Address;  // Current Remote Address
  // std::string m_LAddress;                             // Last Attempted
  ...
  std::map<uint32, uint32>
      _pendingTimeSyncRequests;  // key: counter. value: server time when packet
