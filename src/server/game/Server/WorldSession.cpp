...
bool MapSessionFilter::Process(WorldPacket *packet) {
  ClientOpcodeHandler const *opHandle =
      opcodeTable[static_cast<OpcodeClient>(packet->GetOpcode())];

  // let's check if our opcode can be really processed in Map::Update()
  if (opHandle->ProcessingPlace == PROCESS_INPLACE) return true;

  // we do not process thread-unsafe packets
  if (opHandle->ProcessingPlace == PROCESS_THREADUNSAFE) return false;
...
bool WorldSessionFilter::Process(WorldPacket *packet) {
  ...
  if (opHandle->ProcessingPlace == PROCESS_THREADUNSAFE) return true;
...
WorldSession::WorldSession(uint32 id, std::string &&name,
    ...
    : m_muteTime(mute_time),
      ...
      m_expansion(
          std::min<uint8>(expansion, sWorld->getIntConfig(CONFIG_EXPANSION))),
      _warden(nullptr),
      _logoutTime(0),
      m_inQueue(false),
...
WorldSession::~WorldSession() {
  ...
  while (_recvQueue.next(packet)) delete packet;

  LoginDatabase.PExecute("UPDATE account SET online = 0 WHERE id = %u;",
                         GetAccountId());  // One-time query
}
...
void WorldSession::SendPacket(WorldPacket const *packet,
  ...
  if (packet->GetOpcode() == NULL_OPCODE) {
    LOG_ERROR("network.opcode", "Prevented sending of NULL_OPCODE to %s",
              GetPlayerInfo().c_str());
    return;
  } else if (packet->GetOpcode() == UNKNOWN_OPCODE) {
    LOG_ERROR("network.opcode", "Prevented sending of UNKNOWN_OPCODE to %s",
              GetPlayerInfo().c_str());
    return;
  }

  ServerOpcodeHandler const *handler =
      opcodeTable[static_cast<OpcodeServer>(packet->GetOpcode())];

  if (!handler) {
    LOG_ERROR("network.opcode",
              "Prevented sending of opcode %u with non existing handler to %s",
  ...
  if (!m_Socket) {
    ...
    LOG_ERROR(
        ...
        GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode()))
            .c_str(),
        GetPlayerInfo().c_str());
    return;
  }
  ...
  if (!forced) {
    ...
    if (!handler || handler->Status == STATUS_UNHANDLED) {
      LOG_ERROR("network.opcode", "Prevented sending disabled opcode %s to %s",
                GetOpcodeNameForLogging(
                    static_cast<OpcodeServer>(packet->GetOpcode()))
                    .c_str(),
                GetPlayerInfo().c_str());
      return;
    }
  ...
  if ((cur_time - lastTime) < 60) {
    ...
    LOG_DEBUG("misc",
              "Send all time packets count: " UI64FMTD " bytes: " UI64FMTD
              " avr.count/sec: %f avr.bytes/sec: %f time: %u",
              sendPacketCount, sendPacketBytes,
              float(sendPacketCount) / fullTime,
              float(sendPacketBytes) / fullTime, uint32(fullTime));
    LOG_DEBUG("misc",
              "Send last min packets count: " UI64FMTD " bytes: " UI64FMTD
              " avr.count/sec: %f avr.bytes/sec: %f",
              sendLastPacketCount, sendLastPacketBytes,
...
void WorldSession::LogUnexpectedOpcode(WorldPacket *packet, char const *status,
                                       const char *reason) {
  LOG_ERROR(
      "network.opcode",
      "Received unexpected opcode %s Status: %s Reason: %s from %s",
      GetOpcodeNameForLogging(static_cast<OpcodeClient>(packet->GetOpcode()))
          .c_str(),
      status, reason, GetPlayerInfo().c_str());
}
...
void WorldSession::LogUnprocessedTail(WorldPacket const *packet) {
  ...
  LOG_TRACE(
      "network.opcode",
      "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
...
bool WorldSession::Update(uint32 diff, PacketFilter &updater) {
    ...
    try {
      switch (opHandle->Status) {
        case STATUS_LOGGEDIN:
          if (!_player) {
            // skip STATUS_LOGGEDIN opcode unexpected errors if player logout
            // sometime ago - this can be network lag delayed packets
            //! If player didn't log out a while ago, it means packets are being
            //! sent while the server does not recognize the client to be in
            //! world yet. We will re-add the packets to the bottom of the queue
            //! and process them later.
            if (!m_playerRecentlyLogout) {
              requeuePackets.push_back(packet);
              deletePacket = false;
              LOG_DEBUG("network",
                        "Re-enqueueing packet with opcode %s with with status "
                        "STATUS_LOGGEDIN. "
                        "Player is currently not in world yet.",
                        GetOpcodeNameForLogging(
                            static_cast<OpcodeClient>(packet->GetOpcode()))
                            .c_str());
            }
            ...
            processedPackets =
                MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE;  // break out
                                                                    // of packet
                                                                    // processing
                                                                    // loop
          // lag can cause STATUS_LOGGEDIN opcodes to arrive after the player
          // started a transfer
          break;
        case STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT:
          if (!_player && !m_playerRecentlyLogout &&
              !m_playerLogout)  // There's a short delay between _player = null
                                // and m_playerRecentlyLogout = true during
                                // logout
            LogUnexpectedOpcode(
                packet, "STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT",
        ...
        case STATUS_AUTHED:
          ...
          if (m_inQueue) {
            LogUnexpectedOpcode(packet, "STATUS_AUTHED",
                                "the player not pass queue yet");
            break;
          }

          // some auth opcodes can be recieved before
          // STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT opcodes however when we recieve
          // CMSG_ENUM_CHARACTERS we are surely no longer during the logout
          // process.
        ...
        case STATUS_UNHANDLED:
          LOG_ERROR("network.opcode", "Received not handled opcode %s from %s",
                    GetOpcodeNameForLogging(
                        static_cast<OpcodeClient>(packet->GetOpcode()))
                        .c_str(),
                    GetPlayerInfo().c_str());
      ...
      LOG_ERROR("network",
                "PacketArrayMaxCapacityException: %s while parsing %s from %s.",
                pamce.what(),
                GetOpcodeNameForLogging(
                    static_cast<OpcodeClient>(packet->GetOpcode()))
                    .c_str(),
      ...
      LOG_ERROR(
          "network",
          "WorldSession::Update ByteBufferException occured while parsing a "
          "packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
          packet->GetOpcode(), GetRemoteAddress().c_str(), GetAccountId());
      packet->hexlike();
    }

    if (deletePacket) delete packet;
  ...
  FC_METRIC_VALUE("processed_packets", processedPackets);

  _recvQueue.readd(requeuePackets.begin(), requeuePackets.end());

  if (m_Socket && m_Socket->IsOpen() && _warden) _warden->Update();
  ...
  if (updater.ProcessUnsafe()) {
    ...
    if (m_Socket && GetPlayer() && _warden) _warden->Update();
...
void WorldSession::LogoutPlayer(bool save) {
  ...
  while (_player && _player->IsBeingTeleportedFar()) HandleMoveWorldportAck();
  ...
  if (_player) {
    ...
    for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i) {
      if (BattlegroundQueueTypeId bgQueueTypeId =
              _player->GetBattlegroundQueueTypeId(i)) {
        ...
        BattlegroundQueue &queue =
            sBattlegroundMgr->GetBattlegroundQueue(bgQueueTypeId);
    ...
    // player save
    while (_player->IsBeingTeleportedFar()) HandleMoveWorldportAck();

    ///- If the player is in a guild, update the guild roster and broadcast a
    /// logout message to other guild members
    ...
    if (save) {
      ...
      for (int j = BUYBACK_SLOT_START; j < BUYBACK_SLOT_END; ++j) {
        ...
        _player->SetGuidValue(PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + (eslot * 2),
                              ObjectGuid::Empty);
    ...
    ///- If the player is in a group (or invited), remove him. If the group if
    /// then only 1 person, disband the group.
    ...
    sSocialMgr->SendFriendStatus(_player, FRIEND_OFFLINE, _player->GetGUID(),
                                 true);
    ...
    //! Client will respond by sending 3x CMSG_CANCEL_TRADE, which we currently
    //! dont handle
    ...
    CharacterDatabasePreparedStatement *stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_ACCOUNT_ONLINE);
...
char const *WorldSession::GetFirelandsString(uint32 entry) const {
...
void WorldSession::Handle_EarlyProccess(WorldPacket &recvPacket) {
  LOG_ERROR(
      "network.opcode",
      "Received opcode %s that must be processed in WorldSocket::OnRead from "
      "%s",
...
void WorldSession::Handle_ServerSide(WorldPacket &recvPacket) {
  LOG_ERROR(
      "network.opcode", "Received server-side opcode %s from %s",
      GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode()))
          .c_str(),
      GetPlayerInfo().c_str());
}
...
void WorldSession::Handle_Deprecated(WorldPacket &recvPacket) {
  LOG_ERROR(
      "network.opcode", "Received deprecated opcode %s from %s",
      GetOpcodeNameForLogging(static_cast<OpcodeClient>(recvPacket.GetOpcode()))
          .c_str(),
      GetPlayerInfo().c_str());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask) {
  for (uint32 i = 0; i < NUM_ACCOUNT_DATA_TYPES; ++i)
    if (mask & (1 << i)) m_accountData[i] = AccountData();
  ...
  do {
    ...
    if (type >= NUM_ACCOUNT_DATA_TYPES) {
      LOG_ERROR(
          "misc", "Table `%s` have invalid account data type (%u), ignore.",
    ...
    if ((mask & (1 << type)) == 0) {
      LOG_ERROR(
          "misc",
          "Table `%s` have non appropriate for table  account data "
          "type (%u), ignore.",
          mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data",
          type);
      continue;
...
void WorldSession::SetAccountData(AccountDataType type, time_t tm,
  ...
  if ((1 << type) & GLOBAL_CACHE_MASK) {
    ...
    // _player can be nullptr and packet received after logout but m_GUID still
    // store correct guid
    if (!m_GUIDLow) return;
  ...
  CharacterDatabasePreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(index);
  stmt->setUInt32(0, id);
...
void WorldSession::SendAccountDataTimes(uint32 mask) {
  WorldPacket data(SMSG_ACCOUNT_DATA_TIMES,
                   4 + 1 + 4 + NUM_ACCOUNT_DATA_TYPES * 4);
  ...
  for (uint32 i = 0; i < NUM_ACCOUNT_DATA_TYPES; ++i)
    if (mask & (1 << i))
      data << uint32(
          GetAccountData(AccountDataType(i))->Time);  // also unix time
  SendPacket(&data);
}
...
void WorldSession::SaveTutorialsData(CharacterDatabaseTransaction &trans) {
  if (!(m_TutorialsChanged & TUTORIALS_FLAG_CHANGED)) return;

  bool const hasTutorialsInDB =
      (m_TutorialsChanged & TUTORIALS_FLAG_LOADED_FROM_DB) != 0;
  CharacterDatabasePreparedStatement *stmt =
      CharacterDatabase.GetPreparedStatement(
          hasTutorialsInDB ? CHAR_UPD_TUTORIALS : CHAR_INS_TUTORIALS);
  for (uint8 i = 0; i < MAX_ACCOUNT_TUTORIAL_VALUES; ++i)
  ...
  if (!hasTutorialsInDB) m_TutorialsChanged |= TUTORIALS_FLAG_LOADED_FROM_DB;

  m_TutorialsChanged &= ~TUTORIALS_FLAG_CHANGED;
}

void WorldSession::ReadAddonsInfo(ByteBuffer &data) {
  if (data.rpos() + 4 > data.size()) return;
  ...
  if (size > 0xFFFFF) {
    LOG_DEBUG("addon",
              "WorldSession::ReadAddonsInfo: AddOnInfo too big, size %u", size);
    return;
  }
  ...
  if (uncompress(addonInfo.contents(), &uSize, data.contents() + pos,
                 data.size() - pos) == Z_OK) {
    try {
      ...
      for (uint32 i = 0; i < addonsCount; ++i) {
        ...
        LOG_DEBUG("addon",
                  "AddOn: %s (CRC: 0x%x) - has key: 0x%x - URL CRC: 0x%x",
                  addon.Name.c_str(), publicKeyCrc, addon.HasKey, urlCrc);
        ...
        if (savedAddon) {
          if (publicKeyCrc != savedAddon->CRC) {
            if (addon.HasKey) {
              ...
              LOG_WARN("addon",
                       " Addon: %s: modified (CRC: 0x%x) - accountID %d)",
                       addon.Name.c_str(), savedAddon->CRC, GetAccountId());
            } else
              addon.Status = Addons::SecureAddonInfo::SECURE_HIDDEN;
          } else {
            addon.Status = Addons::SecureAddonInfo::SECURE_HIDDEN;
            LOG_DEBUG("addon",
                      "Addon: %s: validated (CRC: 0x%x) - accountID %d",
                      addon.Name.c_str(), savedAddon->CRC, GetAccountId());
          }
        } else {
          addon.Status = Addons::SecureAddonInfo::BANNED;
          LOG_WARN(
              "addon",
              "Addon: %s: not registered as known secure addon - accountId %d",
              addon.Name.c_str(), GetAccountId());
        }
      }
      ...
      LOG_DEBUG("addon", "AddOn: Newest banned addon timestamp: %u",
                lastBannedAddOnTimestamp);
    } catch (ByteBufferException const &e) {
      LOG_DEBUG("addon", "AddOn: Addon packet read error! %s", e.what());
    }
  } else
    LOG_DEBUG("addon", "AddOn: Addon packet uncompress error!");
}

void WorldSession::SendAddonsInfo() {
  uint8 constexpr addonPublicKey[256] = {
      };

  WorldPacket data(SMSG_ADDON_INFO, 4);

  for (Addons::SecureAddonInfo const &addonInfo : _addons.SecureAddons) {
    // fresh install, not yet created Interface\Addons\addon_name\addon_name.pub
    // files
    uint8 infoProvided =
        addonInfo.Status != Addons::SecureAddonInfo::BANNED || addonInfo.HasKey;
    ...
    if (infoProvided) {
      ...
      if (!addonInfo
               .HasKey)  // if CRC is wrong, add public key (client need it)
      {
        LOG_DEBUG("addon",
                  "AddOn: %s: key missing: sending pubkey to accountID %d",
                  addonInfo.Name.c_str(), GetAccountId());

        data.append(addonPublicKey, sizeof(addonPublicKey));
      }

      data << uint32(
          0);  // Revision (from .toc), can be used by SECURE_VISIBLE to display
               // "update available" in client addon controls
    }

    data << uint8(0);  // UrlProvided
                       // if (usesURL)
    //     data << uint8(0);                                     // URL, client
    //     will create internet shortcut with this destination in
    //     Interface\Addons\addon_name\addon_name.url
  }
  ...
  if (!bannedAddons->empty() &&
      bannedAddons->back().Timestamp <
          lastBannedAddOnTimestamp)  // cheating attempt OR connecting to a
                                     // realm with different configured banned
                                     // addons, send everything
  ...
  data << uint32(0);
  auto itr =
      std::lower_bound(bannedAddons->begin(), bannedAddons->end(),
                       _addons.LastBannedAddOnTimestamp,
                       [](BannedAddon const &bannedAddon, uint32 timestamp) {
...
bool WorldSession::IsAddonRegistered(const std::string &prefix) const {
  if (!_filterAddonMessages)  // if we have hit the softcap (64) nothing should
                              // be filtered
    return true;

  if (_registeredAddonPrefixes.empty()) return false;

  std::vector<std::string>::const_iterator itr = std::find(
      _registeredAddonPrefixes.begin(), _registeredAddonPrefixes.end(), prefix);
  return itr != _registeredAddonPrefixes.end();
}

void WorldSession::HandleUnregisterAddonPrefixesOpcode(
    WorldPacket & /*recvPacket*/)  // empty packet
{
  LOG_DEBUG("network", "WORLD: Received CMSG_UNREGISTER_ALL_ADDON_PREFIXES");

  _registeredAddonPrefixes.clear();
}

void WorldSession::HandleAddonRegisteredPrefixesOpcode(
    WorldPacket &recvPacket) {
  LOG_DEBUG("network", "WORLD: Received CMSG_ADDON_REGISTERED_PREFIXES");
  ...
  for (uint32 i = 0; i < count; ++i) lengths[i] = recvPacket.ReadBits(5);

  for (uint32 i = 0; i < count; ++i)
    _registeredAddonPrefixes.push_back(recvPacket.ReadString(lengths[i]));

  if (_registeredAddonPrefixes.size() >
      REGISTERED_ADDON_PREFIX_SOFTCAP)  // shouldn't happen
  {
    _filterAddonMessages = false;
    return;
  }
...
void WorldSession::SetPlayer(Player *player) {
  ...
  // set m_GUID that can be used while player loggined and later until
  // m_playerRecentlyLogout not reset
  if (_player) m_GUIDLow = _player->GetGUID().GetCounter();
}
...
TransactionCallback &WorldSession::AddTransactionCallback(
    TransactionCallback &&callback) {
  return _transactionCallbacks.AddCallback(std::move(callback));
}

SQLQueryHolderCallback &WorldSession::AddQueryHolderCallback(
...
class AccountInfoQueryHolderPerRealm : public CharacterDatabaseQueryHolder {
 public:
  ...
  bool Initialize(uint32 accountId) {
    ...
    CharacterDatabasePreparedStatement *stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_ACCOUNT_DATA);
...
void WorldSession::InitializeSession() {
  ...
  AddQueryHolderCallback(CharacterDatabase.DelayQueryHolder(realmHolder))
      .AfterComplete([this](SQLQueryHolderBase const &holder) {
        InitializeSessionCallback(
            static_cast<AccountInfoQueryHolderPerRealm const &>(holder));
      });
}

void WorldSession::InitializeSessionCallback(
    CharacterDatabaseQueryHolder const &realmHolder) {
  LoadAccountData(realmHolder.GetPreparedResult(
                      AccountInfoQueryHolderPerRealm::GLOBAL_ACCOUNT_DATA),
                  GLOBAL_CACHE_MASK);
  LoadTutorialsData(
      realmHolder.GetPreparedResult(AccountInfoQueryHolderPerRealm::TUTORIALS));
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket &p,
  ...
  if (!maxPacketCounterAllowed) return true;
  ...
  if (++packetCounter.amountCounter <= maxPacketCounterAllowed) return true;

  LOG_WARN("network",
           "AntiDOS: Account %u, IP: %s, Ping: %u, Character: %s, flooding "
           "packet (opc: %s (0x%X), count: %u)",
           Session->GetAccountId(), Session->GetRemoteAddress().c_str(),
           Session->GetLatency(), Session->GetPlayerName().c_str(),
           opcodeTable[static_cast<OpcodeClient>(p.GetOpcode())]->Name,
           p.GetOpcode(), packetCounter.amountCounter);
  ...
  switch (_policy) {
    ...
    case POLICY_BAN: {
      ...
      uint32 duration =
          sWorld->getIntConfig(CONFIG_PACKET_SPOOF_BANDURATION);  // in seconds
      std::string nameOrIp = "";
      switch (bm) {
        case BAN_CHARACTER:  // not supported, ban account
        case BAN_ACCOUNT:
          (void)sAccountMgr->GetName(Session->GetAccountId(), nameOrIp);
          break;
        case BAN_IP:
          nameOrIp = Session->GetRemoteAddress();
          break;
      }
      sWorld->BanAccount(bm, nameOrIp, duration,
                         "DOS (Packet Flooding/Spoofing", "Server: AutoDOS");
      LOG_WARN("network",
               "AntiDOS: Player automatically banned for %u seconds.",
               duration);
      Session->KickPlayer();
      return false;
    }
...
uint32 WorldSession::DosProtection::GetMaxPacketCounterAllowed(
  ...
  switch (opcode) {
    ...
    case CMSG_TRAINER_BUY_SPELL:                  // not profiled
    {
      ...
      // All the opcodes above must cause little CPU usage and no sync/async
    ...
    case CMSG_SET_CONTACT_NOTES:      //   1               2.5       1 async db
                                      //   query
    case CMSG_CALENDAR_GET_CALENDAR:  //   0               1.5       medium
                                      //   upload bandwidth usage
    case CMSG_GUILD_BANK_QUERY_TAB:   //   0               3.5       medium
                                      //   upload bandwidth usage
    case CMSG_QUERY_INSPECT_ACHIEVEMENTS:  //   0              13         high
                                           //   upload bandwidth usage
    ...
    case CMSG_QUEST_POI_QUERY:  //   0              25         very high upload
                                //   bandwidth usage
    {
    ...
    case CMSG_PETITION_SIGN:  //   9               4         2 sync 1 async db
                              //   queries
    case CMSG_TURN_IN_PETITION:  //   8               5.5       2 sync db query
    case CMSG_GROUP_CHANGE_SUB_GROUP:  //   6               5         1 sync 1
                                       //   async db queries
    ...
    case CMSG_CHAR_FACTION_CHANGE:  //   5               5         1 sync db
                                    //   query
    ...
    case CMSG_GROUP_RAID_CONVERT:      //   1               5         1 async db
                                       //   query
    case CMSG_GROUP_ASSISTANT_LEADER:  //   1               2         1 async db
                                       //   query
    case CMSG_CALENDAR_ADD_EVENT:      //  21              10         2 async db
                                       //  query
    case CMSG_PETITION_BUY:  // not profiled                1 sync 1 async db
                             // queries
    ...
    case CMSG_ENUM_CHARACTERS:  //  22               3         2 async db
                                //  queries
    case CMSG_GMTICKET_CREATE:  //   1              25         1 async db query
    case CMSG_GMTICKET_UPDATETEXT:    //   0              15         1 async db
                                      //   query
    case CMSG_GMTICKET_DELETETICKET:  //   1              25         1 async db
                                      //   query
    case CMSG_GMRESPONSE_RESOLVE:     //   1              25         1 async db
                                      //   query
...
WorldSession::DosProtection::DosProtection(WorldSession *s)
    : Session(s),
      _policy((Policy)sWorld->getIntConfig(CONFIG_PACKET_SPOOF_POLICY)) {}
...
void WorldSession::SendTimeSync() {
  ...
  // Schedule next sync in 5 sec (sniffs sometimes vary between 5 and 6 seconds.
  // Probably due to their 400 batch interval)
  _timeSyncTimer = 5000;
...
bool WorldSession::IsRightUnitBeingMoved(ObjectGuid guid) {
  ...
  // edit: this wouldn't happen in retail but it does in TC, even with a
  // legitimate client.
  if (!client->GetActivelyMovedUnit() ||
      client->GetActivelyMovedUnit()->GetGUID() != guid) {
    LOG_DEBUG("entities.unit",
              "Attempt at tampering movement data by Player %s",
              _player->GetName().c_str());
    return false;
  }

  // This can happen if a legitimate client has lost control of a unit but
  // hasn't received SMSG_CONTROL_UPDATE before sending this packet yet. The
  // server should silently ignore all MOVE messages coming from the client as
  // soon as control over that unit is revoked (through a 'SMSG_CONTROL_UPDATE
  // allowMove=false' message).
  if (!client->IsAllowedToMove(guid)) {
    LOG_DEBUG("entities.unit", "Bad or outdated movement data by Player %s",
              _player->GetName().c_str());
    return false;
  }
