...
DatabaseLoader::DatabaseLoader(std::string const& logger, uint32 const defaultUpdateMask, std::string_view modulesList)
    : _logger(logger), _modulesList(modulesList), _autoSetup(sConfigMgr->GetBoolDefault("Updates.AutoSetup", true)),
...
template <class T> DatabaseLoader& DatabaseLoader::AddDatabase(DatabaseWorkerPool<T>& pool, std::string const& name)
{
    ...
    _open.push(
        [this, name, updatesEnabledForThis, &pool]() -> bool
        {
            ...
            if (uint32 error = pool.Open())
            {
                ...
                if (error)
                {
                    LOG_ERROR("sql.driver",
                        ...
                        "for specific errors. Read wiki at https://firelandsproject.github.io/wiki/home",
                        name.c_str());
...
template FC_DATABASE_API DatabaseLoader& DatabaseLoader::AddDatabase<LoginDatabaseConnection>(DatabaseWorkerPool<LoginDatabaseConnection>&, std::string const&);
template FC_DATABASE_API DatabaseLoader& DatabaseLoader::AddDatabase<CharacterDatabaseConnection>(DatabaseWorkerPool<CharacterDatabaseConnection>&, std::string const&);
template FC_DATABASE_API DatabaseLoader& DatabaseLoader::AddDatabase<WorldDatabaseConnection>(DatabaseWorkerPool<WorldDatabaseConnection>&, std::string const&);
template FC_DATABASE_API DatabaseLoader& DatabaseLoader::AddDatabase<HotfixDatabaseConnection>(DatabaseWorkerPool<HotfixDatabaseConnection>&, std::string const&);
template FC_DATABASE_API DatabaseLoader& DatabaseLoader::AddDatabase<PlayerbotsDatabaseConnection>(DatabaseWorkerPool<PlayerbotsDatabaseConnection>&, std::string const&);
