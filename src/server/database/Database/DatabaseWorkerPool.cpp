      ...
      _async_threads(0), _synch_threads(0)
{
    ...
    WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "Firelands does not support MySQL versions below 5.1");
    WPFatal(mysql_get_client_version() == MYSQL_VERSION_ID, "Used MySQL library version (%s id %lu) does not match the version id used to compile Firelands (id %u). Search on forum for FCE00011.",
...
{
    _connectionInfo = Firelands::make_unique<MySQLConnectionInfo>(infoString);
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        auto connection = [&] {
            ...
            {
            case IDX_ASYNC:
                return Firelands::make_unique<T>(_queue.get(), *_connectionInfo);
            case IDX_SYNCH:
                return Firelands::make_unique<T>(*_connectionInfo);
            default:
                ABORT();
            }
        }();
        ...
        {
            LOG_ERROR("sql.driver", "Firelands does not support MySQL versions below 5.1");
            return 1;
        }
        else
        {
...
{
    if (Firelands::IsFormatEmptyOrNull(sql))
        return;
